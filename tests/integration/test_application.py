...
async def test_refresh_charmhub_to_local():
    ...
    async with base.CleanModel() as model:
        app = await model.deploy('ubuntu', application_name='ubu-path', channel='stable', revision=23)
        await app.refresh(path=str(charm_path))
        assert app.data['charm-url'].startswith('local:')

        app = await model.deploy('ubuntu', application_name='ubu-switch', channel='stable', revision=23)
...
async def test_local_refresh():
    charm_path = INTEGRATION_TEST_DIR / 'charm'
    async with base.CleanModel() as model:
        app = await model.deploy('ubuntu', channel='stable', revision=23)
