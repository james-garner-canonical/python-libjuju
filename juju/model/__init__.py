...
class ModelState:
    ...
    def apply_delta(self, delta):
        ...
        """
        history = self.state.setdefault(delta.entity, {}).setdefault(
            delta.get_id(), collections.deque()
        )
    ...
    def get_entity(
        ...
        return entity_class(
            entity_id, self.model, history_index=history_index, connected=connected
        )
...
class ModelEntity:
    ...
    def entity_type(self):
        ...
        if hasattr(self.__class__, "type_name_override") and callable(
            self.__class__.type_name_override
        ):
    ...
    def dead(self):
        ...
        return (
            self.data is None
            or self.model.state.entity_data(self.entity_type, self.entity_id, -1)
            is None
        )
    ...
    def previous(self):
        ...
        return self.model.state.get_entity(
            self.entity_type, self.entity_id, self._history_index - 1, connected=False
        )
    ...
    def next(self):
        ...
        connected = (
            new_index
            == len(self.model.state.entity_history(self.entity_type, self.entity_id))
            - 1
        )
        return self.model.state.get_entity(
            self.entity_type,
        ...
        if app_name is None:
            if is_bundle:
                bundle_with_overlays = [
                    b for b in yaml.safe_load_all(bundle_path.read_text())
                ]
...
class CharmhubDeployType:
    ...
    async def resolve(
        ...
        if revision and not channel:
            raise JujuError(
                "specifying a revision requires a channel for future upgrades. Please use --channel"
            )
        ...
        charm_url, origin = await self.charm_resolver(
            url, origin, force, series, model_conf
        )

        is_bundle = origin.type_ == "bundle"
        if is_bundle and revision and channel:
            raise JujuError(
                "revision and channel are mutually exclusive when deploying a bundle. Please choose one."
            )
...
class Model:
    ...
    async def connect(self, *args, **kwargs):
        ...
        if "endpoint" not in kwargs and len(args) < 2:
            ...
            if "model_name" in kwargs:
                raise TypeError("connect() got values for both model_name and endpoint")
            ...
            has_userpass = len(args) >= 4 or {"username", "password"}.issubset(kwargs)
            ...
            if not {"endpoint", "uuid"}.issubset(kwargs):
                raise ValueError(
                    "endpoint and uuid are required if model_name not given"
                )
            if not (
                ...
                raise ValueError(
                    "Authentication parameters are required if model_name not given"
                )
    ...
    async def _after_connect(self, model_name=None, model_uuid=None):
        ...
        if self._info is None:
            ...
            # be done lazily (i.e. not every time after_connect, but whenever
    ...
    async def reset(self, force=False):
        ...
        for app in self.applications.values():
            await app.destroy()
        await self.block_until(lambda: len(self.applications) == 0)
        for machine in self.machines.values():
            await machine.destroy(force=force)
        await self.block_until(lambda: len(self.machines) == 0)
    ...
    async def create_storage_pool(self, name, provider_type, attributes=""):
        ...
        return await storage_facade.CreatePool(
            pools=[
                client.StoragePool(
                    name=name, provider=provider_type, attrs=dict(_attrs)
                )
            ]
        )
    ...
    async def remove_storage_pool(self, name):
        ...
        """
        storage_facade = client.StorageFacade.from_connection(self.connection())
        return await storage_facade.RemovePool(
            pools=[client.StoragePoolDeleteArg(name)]
        )
    ...
    async def update_storage_pool(self, name, attributes=""):
        ...
        storage_facade = client.StorageFacade.from_connection(self.connection())
        return await storage_facade.UpdatePool(
            pools=[
                client.StoragePool(
                    name=name,
                    attrs=_attrs,
    ...
    async def list_storage(self, filesystem=False, volume=False):
        ...
        if filesystem:
            _res = await storage_facade.ListFilesystems(
                filters=[client.FilesystemFilter()]
            )
        elif volume:
            _res = await storage_facade.ListVolumes(filters=[client.VolumeFilter()])
        else:
            _res = await storage_facade.ListStorageDetails(
                filters=[client.StorageFilter()]
            )
    ...
    async def show_storage_details(self, *storage_ids):
        ...
        storage_facade = client.StorageFacade.from_connection(self.connection())
        res = await storage_facade.StorageDetails(
            entities=[client.Entity(tag.storage(s)) for s in storage_ids]
        )
    ...
    async def remove_storage(self, *storage_ids, force=False, destroy_storage=False):
        ...
        storage_facade = client.StorageFacade.from_connection(self.connection())
        ret = await storage_facade.Remove(
            storage=[
                client.RemoveStorageInstance(
    ...
    async def remove_application(
        ...
        if app_name not in self.applications:
            raise JujuError(
                "Given application doesn't seem to appear in the\
        ...
        if block_until_done:
            await self.block_until(
                lambda: app_name not in self.applications, timeout=timeout
            )
    ...
    def add_observer(
        self, callable_, entity_type=None, action=None, entity_id=None, predicate=None
    ):
        ...
        """
        observer = _Observer(callable_, entity_type, action, entity_id, predicate)
    ...
    async def _notify_observers(self, delta, old_obj, new_obj):
        ...
        log.debug("Model changed: %s %s %s", delta.entity, delta.type, delta.get_id())
    ...
    async def integrate(self, relation1, relation2):
        ...
        if remote_endpoint is not None:
            ...
            await self.consume(
                remote_endpoint.string(), application_alias=remote_endpoint.application
            )

        log.debug("Adding relation %s <-> %s", endpoints[0], endpoints[1])
    ...
    async def add_space(self, name, cidrs=None, public=True):
        ...
        spaces = [{"cidrs": cidrs, "space-tag": tag.space(name), "public": public}]
    ...
    async def create_backup(self, notes=None):
        ...
        if "error" in backup_metadata:
            raise JujuBackupError(
                "unable to create a backup, got %s from Juju API" % backup_metadata
            )
    ...
    async def deploy(
        ...
        if res.is_bundle:
            ...
            if overlays:
                raise JujuError(
                    "options provided but not supported when deploying a charm: overlays=%s"
                    % overlays
                )
            ...
            if not res.is_local:
                ...
                if isinstance(add_charm_res, dict):
                    ...
                    charm_origin = add_charm_res.get("charm_origin", charm_origin)
                else:
                ...
                if Schema.CHARM_HUB.matches(schema):
                    if (
                        ...
                        resources = await self._add_charmhub_resources(
                            res.app_name, identifier, add_charm_res.charm_origin
                        )
                    ...
                    if is_sub:
                        if num_units > 1:
                            raise JujuError(
                                "cannot use num_units with subordinate application"
                            )
                        num_units = 0
                ...
                resources = await self.add_local_resources(
                    application_name, identifier, metadata, resources=resources
                )
    ...
    async def _add_charm(self, charm_url, origin):
        ...
        if charms_cls.best_facade_version(self.connection()) > 2:
            ...
            return await charms_facade.AddCharm(
                charm_origin=origin, url=charm_url, force=False
            )

        client_facade = client.ClientFacade.from_connection(self.connection())
        return await client_facade.AddCharm(
            channel=str(origin.risk), url=charm_url, force=False
        )
    ...
    async def _add_charmhub_resources(
        ...
        resource_map = {
            resource["name"]: pid
            for resource, pid in zip(resources, response.pending_ids or {})
        }
    ...
    async def _deploy(
        ...
        if server_side_deploy:
            ...
            for _result in result.results:
                for pending_upload_resource in getattr(
                    _result, "pendingresourceuploads", []
                ):
    ...
    async def destroy_unit(
        self, unit_id, destroy_storage=False, dry_run=False, force=False, max_wait=None
    ):
        """Destroy units by name."""
        return await self.destroy_units(
            unit_id,
            destroy_storage=destroy_storage,
            dry_run=dry_run,
            force=force,
    ...
    def download_backup(self, archive_id, target_filename=None):
        ...
        if not response.status == 200:
            raise JujuBackupError(
                "unable to download the backup ID : %s -- got : %s from the JujuAPI with a HTTP response code : %s"
                % (archive_id, result, response.status)
            )
    ...
    def restore_backup(
        ...
        raise DeprecationWarning(
            "juju restore-backup is deprecated in favor of the stand-alone 'juju-restore' tool: https://github.com/juju/juju-restore"
        )
    ...
    async def set_config(self, config):
        ...
        for key, value in config.items():
            if isinstance(value, client.ConfigValue):
                ...
                raise JujuModelConfigError(
                    "Expected either a string or a ConfigValue as a config value, found : %s of type %s"
                    % (value, type(value))
                )
    ...
    async def _get_completed_action(self, action_uuid, wait=None):
        ...
        # has already occurred then the delta has gone.
    ...
    async def get_metrics(self, *tags):
        ...
        """
        log.debug("Retrieving metrics for %s", ", ".join(tags) if tags else "all units")

        metrics_facade = client.MetricsDebugFacade.from_connection(self.connection())
    ...
    async def create_offer(self, endpoint, offer_name=None, application_name=None):
        ...
        async with ConnectedController(self.connection()) as controller:
            return await controller.create_offer(
                self.info.uuid,
                endpoint,
    ...
    async def consume(
        ...
        arg = _create_consume_args(
            consume_details.offer,
...
class CharmArchiveGenerator:
    ...
    def _check_link(self, path: str) -> None:
        ...
        if link_path[0] == "/":
            raise ValueError(
                "Invalid Charm at %s: %s" % (path, "Absolute links are invalid")
            )
